...
test("resourceKeys flat", (t) => {
  ...
  t.deepEqual(i18n.resourceKeys("en"), ["greeting"]);
});
...
test("availableLocales", (t) => {
  ...
  t.deepEqual(i18n.availableLocales(), ["en", "ru"]);
});

test("missingKeys ", (t) => {
  const i18n = createMultiLanguageExample();
  t.deepEqual(i18n.missingKeys("en", "ru"), []);
  t.deepEqual(i18n.missingKeys("ru"), ["checkout"]);
});

test("overspecifiedKeys", (t) => {
  const i18n = createMultiLanguageExample();
  t.deepEqual(i18n.overspecifiedKeys("ru"), []);
  t.deepEqual(i18n.overspecifiedKeys("en", "ru"), ["checkout"]);
});
